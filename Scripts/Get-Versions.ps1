#IMPORTANT: This script has issues with Content Patcher Packs, I'm working on fixing it, but that might be a while.
...
{
    $mods += (Get-Content (Join-Path $d.FullName -ChildPath "manifest.json") | Out-String | ConvertFrom-Json)
}
